...
export default function Task({ task, onToggle, handleDelete, onEdit }) {
  ...
  <li className={`${task.completed ? "underline" : ""} bg-blue-300 rounded-md m-4 p-2 ${afterDeadline ? "bg-red-200" : ""}   `} onDoubleClick={() => setIsEditing(true)}>
    {isEditing ? (
      <div className='grid grid-cols-2 grid-rows-4 gap-4'>
        <input
          className="bg-gray-300 outline-2 outline-blue-700 p-2 text-sm focus:outline-4 focus:outline-emerald-500 col-span-2"
          ref={inputRef}
          value={editedText}
        ...
        <textarea
          ...
          value={editedArea}
          className="col-span-2 bg-gray-300 outline-2 outline-blue-700 p-2 text-sm focus:outline-4 focus:outline-emerald-500"
        />
        <select
